  ...
  return new Promise((resolve, reject) => {
    ...
    pWork.push(new Promise((resolve1, reject1) => {
      ...
      rest.get ( canvasobj.client.instanceUrl +  canvasobj.context.links.queryUrl, {
        ...
        query: {
          q:`select Id, Name, Leavers__c, P_L__c from Bid_P_L__c where Bid__c = '${canvasobj.context.environment.record.Id}' ORDER BY Id DESC LIMIT 5`
        }
      }).on('complete', (sfrecs) => {
    ...
    pWork.push (new Promise((resolve2, reject2) => {
      ...
      rest.get ( canvasobj.client.instanceUrl +  canvasobj.context.links.queryUrl, {
        ...
        query: {
          q:`select Id, Bid__r.Name, Bid__r.Contract_Term__c,  Router_Selection__r.Name, Router_Selection__r.Cost__c, Router_Selection__r.Markup__c, Access_Method__r.Name, Access_Method__r.One_Off_Cost__c, Access_Method__r.Recurring_Cost__c, Access_Method__r.Markup__c, Bandwidth__c, Bid_Port_Speed__r.Cost__c, Bid_Port_Speed__r.Markup__c from Bid_Site__c where Bid__c = '${canvasobj.context.environment.record.Id}'`
        }
      }).on('complete', (sfrecs) => {
...
function calcpnl (sfrecs, leavers) {
  ...
  if (sfrecs && sfrecs.length >0) for (let r of sfrecs) {
    if (!r.Router_Selection__r) r.Router_Selection__r = {Cost__c: 0, Markup__c:0};
    if (!r.Bid_Port_Speed__r) r.Bid_Port_Speed__r = {Cost__c: 0, Markup__c:0};
    if (!r.Access_Method__r) r.Access_Method__r = {Recurring_Cost__c: 0, One_Off_Cost__c:0, Markup__c:0};

    let router_1cost = r.Router_Selection__r.Cost__c,
        router_price = router_1cost + ((r.Router_Selection__r.Markup__c /100) * router_1cost),
        port_1cost = r.Bid_Port_Speed__r.Cost__c,
        port_price = port_1cost + ((r.Bid_Port_Speed__r.Markup__c /100) * port_1cost),
        access_1cost = r.Access_Method__r.One_Off_Cost__c,
        access_ncost = r.Access_Method__r.Recurring_Cost__c * leavers.term,
        access_price = access_1cost  + ((r.Access_Method__r.Markup__c /100) * access_1cost)  + access_ncost  + ((r.Access_Method__r.Markup__c / 100) * access_ncost),
      ...
      if (err)
        ...
        if (!save) {
          if (req.body.leavers) {
            ...
            let newleavers = JSON.parse(JSON.parse(data.versions).find((v) => v.Id === req.body.version).Leavers__c);
          ...
          rest.postJson ( canvasobj.client.instanceUrl +  canvasobj.context.links.sobjectUrl + 'Bid_P_L__c',
            {
              "Bid__c": canvasobj.context.environment.record.Id,
              "Leavers__c": JSON.stringify(req.body.leavers),
              "P_L__c": JSON.stringify(pnl)
            },{
            headers: {
              'Authorization': 'Bearer ' +canvasobj.client.oauthToken,
            } ,
