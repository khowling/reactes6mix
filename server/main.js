  ...
  return new Promise((resolve, reject) => {
    ...
    rest.get ( canvasobj.client.instanceUrl +  canvasobj.context.links.queryUrl, {
      ...
      if (sfrecs.records && sfrecs.records.length >0) {
        ...
        redis.lpush(key, JSON.stringify({
            pnl: pnl,
            leavers: leavers,
            sfrecs: sfrecs.records
          }), (succ) => {resolve(succ)});
      }
    }).on('error', (err) => {
